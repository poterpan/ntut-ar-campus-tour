...
> 提案週已結束,進入 4 週開發期 W1。詳細分工與 W1 todo 見 `docs/分工.md`。

**本週(W1 開發起手)**:
1. A 建 Unity 專案 `unity-app/` 推 GitHub、把 Geospatial Sample 搬過來跑通
2. 全員一起實地走新生南路側門路徑,收集 5 個 POI 座標
3. 組會拍板:B/C/D 三人認領模組、貓觸發機制三選一
4. B 起步 LLM API、C 起步 NPC 模型與 POI 短版、D 起步 RL 訓練場景

**接下來(W2–W4)**:
- W2 平行開發各模組
- W3 整合
- W4 收尾(實機測試、UI、Demo 影片、結案報告)
