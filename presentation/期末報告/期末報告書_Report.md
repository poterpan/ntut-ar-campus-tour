...
完整 Figma 設計彙整(資訊架構、視覺風格、HUD 配置、各畫面預覽與最終建議方向)如下,互動原型見 [Figma 連結](https://www.figma.com/proto/l7W1rVAnNVR8hHfwBFkHPN/AR?node-id=25-2):
...
老實說這次最大的感想就是「整合比寫功能還累」。功能各自寫其實都還好,但要把 AR、LLM、語音、RL 全部塞進同一個 App、還要 iOS 跟 Android 兩邊都能跑,中間真的一堆雷。像那個相機黑畫面我 debug 超久,後來才發現是我們把 Google 範例的隱私窗拿掉的時候,連帶把啟動 AR 的那一行也弄不見了,看到原因的當下有夠想吐血。不過 CI/CD 設起來之後超爽,commit 完手機馬上就有新版可以測,這個真的值得。

**簡妤真**

我最有印象的是做那隻 3D 導遊。本來想說 AI 生個圖、丟 Meshy 就好應該很快,結果綁完骨頭手會黏到大腿,動起來超恐怖哈哈,試了好幾次最後是丟一張 T-pose 的圖才解決。UI 我在 Figma 排了好幾版,暖色加玻璃那種感覺我自己蠻喜歡的;看到實機跑出來真的有對到設計稿的時候,還是很有成就感。
...
校園貓是我做的。一開始想說用 ML-Agents,畢竟之前作業有訓練過那隻蜂鳥,但評估一下發現我們這個「找最近的罐頭」其實超單純,根本不用搞到神經網路,就改用最基本的 tabular Q-Learning 自己刻,訓練個一千三百回合貓就會乖乖去吃罐頭了。模型小到只是一個 json,放進 App 完全沒負擔。簡單但有效,我覺得這種取捨還蠻務實的。
