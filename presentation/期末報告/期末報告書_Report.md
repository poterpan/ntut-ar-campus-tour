...
![北科新生南路側門入口實景(導覽路線起點)](img-gate.jpg)

## 1.4 課程要求對齊

| 課程要求 | 本專案對應 |
|---|---|
| 校園相關導覽 App | 北科校園 AR 導覽 |
| 基於 Unity 的 AR App | Unity 6 + AR Foundation + ARCore Extensions |
| 包含一個「訓練出來的」NPC | AR 校園貓(Q-Learning 訓練 1300 episodes;經確認任何訓練出的 model 皆可,不限定蜂鳥 Hummingbirds) |
...
- **簡妤真**:虛擬導遊 NPC(角色、動畫狀態機與解說流程);以及全 App 的 UI/UX 設計(Figma 設計稿與暖色毛玻璃設計系統)。
...
![系統三層架構:手機端 Client／雲端服務 Cloud／訓練端 Training](架構圖.png)
...
![asmdef 模組相依（箭頭表示「依賴」;Poi 為共用基礎層,Cat 與主線解耦）](fig-modules.png)
...
![ARCore Geospatial:GPS + VPS + IMU/SLAM 三源融合定位](fig-geospatial.png)
...
![POI 接近觸發流程(走到 30m 內,導遊自動現身)](fig-proximity.png)
...
導遊 NPC 為 **Jenson**(以 Meshy 生成的 biped,皮衣眼鏡造型,暱稱「老黃」)。採 Generic rig,具四組動畫:**聆聽(Listening)、說話(Talk)、走動(Walking)、揮手(Wave)**,由 `NpcAnimator` 包裝 Animator,提供 `PlayGreet()` / `PlayListening()` / `PlayTalk()` 等觸發。
...
- `GuideChatPanel`:畫面下半的對話面板(毛玻璃風格),含訊息歷史、輸入框、送出鍵與麥克風鍵,並負責顯示「老黃思考中…」「正在生成語音…」等狀態。

![導遊 NPC 動畫狀態機（揮手 / 聆聽 / 說話,另有走動 Walking 用於移動）](fig-npc-states.png)
...
### 3.2.3 UI/UX 設計(Figma + 暖色毛玻璃設計系統)

> 本節的視覺設計稿由組員以 **Figma** 製作,**此處預留設計稿插圖專區**(報告定稿時插入)。

App 採「**皮克敏暖色 + 毛玻璃(glassmorphism)**」設計系統,以程式化 UI builder(`Scripts/Editor/UiBuilder/`)產生 prefab,確保版控可控、風格一致。設計 token(節錄自 `UiPalette`):

| Token | 色值 | 用途 |
|---|---|---|
| GlassFill | #FFFCF5 @ 75% | 毛玻璃面板底 |
| TextMain | #5D4037(深棕) | 主要文字 |
| ButtonGreen | #AED581 | NPC 徽章、操作鈕 |
| AccentOrange | #F57C00 | 強調色 |
| WarmBg(上/下) | #F7F1E5 / #EDDFC8 | 暖色漸層背景 |

主要畫面:**開場(Onboarding)/ HUD(AR 主視圖)/ POI 抽屜 / 探索手帳**。簡報將以實機畫面呈現實際 UI。
...
![對話 + 語音 pipeline:STT → 簡繁轉換 → LLM →（文字先上）→ TTS → 播放,含重試 / 降級](fig-pipeline.png)
...
![RL 狀態離散化:以貓為中心,4 距離環 × 8 角度扇形 = 32 離散狀態](fig-rl-states.png)
...
![Q-Learning 決策與更新迴圈（觀察 → 選動作 → 執行 → reward → 更新 Q）](fig-qloop.png)
...
![CI/CD 自動化建置:push → Unity 匯出 → 雙平台簽名 / 發佈](fig-cicd.png)

![GitHub Actions 雙平台建置成功](img-ci.png)

![iOS 自動上架 TestFlight](img-testflight.png)
...
> 依課程報告慣例,每個問題以 **症狀 / 成因 / 解法** 表呈現,並附關鍵 before/after。
...
```csharp
// After:StartupPermissionRequester(關鍵)
PlayerPrefs.SetInt("HasDisplayedGeospatialPrivacyPrompt", 1); // → OnEnable 直接 SwitchToARView(true) 啟用 AR
#if UNITY_IOS
...
| 2026-06-11 | 真 LLM 接入(Phase 2b);UI/UX 全面打磨(暖色毛玻璃);導遊換 Jenson 模型 |
